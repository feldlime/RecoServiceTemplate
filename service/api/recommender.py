...
class Recommender:
    def __init__(self, dataset_path: str, warm_model_path: str, hot_model_path: str):
        ...
        # df_hot = pd.DataFrame({'user_id':
        # self.interactions[self.interactions['user_id'].isin(self.hot_users)]["user_id"]})
        # self.recos_hot = self.hot_model.predict(df_hot)
        #
        # print("Hot recos predicted")
        #
        # df_warm = self.interactions[
        # ~self.interactions['user_id'].isin(df_hot['user_id'])].drop_duplicates(subset='user_id')
    ...
    def recommend(self, user_id: int, k_recs: int):
        ...
        if self.hot_users.isin([user_id]).any():
            ...
            # recos = self.recos_hot[
            # self.recos_hot['user_id'].isin([user_id])]["item_id"]
            # print(f"user_id {user_id} is hot;
            # recos {recos}; len{len(recos)}")
        ...
        if user_id not in self.missing_user_id_values:
            ...
            #     k=k_recs,
            #     filter_viewed=True)
            # recos = self.recos_warm[
            # self.recos_warm['user_id'].isin([user_id])]["item_id"]
