...
def user_based_recsys(user_id: int, n_recommendations: int = 10) -> List[int]:
...
def find_top_movies(n: int = 10) -> List[int]:
...
def complete_array_to_ten(recommendations: List[int], top_movies: List[int]) -> List[int]:
...
def get_reccomendation(user_id: int, n_recommendations: int = 10) -> List[int]:
    rec = user_based_recsys(user_id)
    if len(rec) == 10:
        return rec
    popular = find_top_movies(n=n_recommendations)
